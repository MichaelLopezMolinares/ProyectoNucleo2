...
class AsignaturaRepository {
  async findAll(programaId = null) {
    ...
    return result.map(r => new Asignatura(r));
  }

  async findById(id) {
    const result = await sequelize.query('SELECT * FROM asignaturas WHERE id = $1', [id]);
    return result[0] ? new Asignatura(result[0]) : null;
  }

  async findByCodigo(codigo) {
    const result = await sequelize.query('SELECT * FROM asignaturas WHERE codigo = $1', [codigo]);
    return result[0] ? new Asignatura(result[0]) : null;
  }
  ...
  async create({ codigo, nombre, creditos, horasSemanales, semestre, programaId }) {
    ...
    return new Asignatura(result[0]);
  }
  ...
  async delete(id) {
    const result = await sequelize.query('DELETE FROM asignaturas WHERE id = $1', [id]);
