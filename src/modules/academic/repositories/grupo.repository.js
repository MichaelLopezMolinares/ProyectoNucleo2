...
class GrupoRepository {
  async findAll(filters = {}) {
    ...
    return result.rows.map(r => new Grupo(r));
  }

  async findById(id) {
    const result = await query('SELECT * FROM grupos WHERE id = $1', [id]);
    return result.rows[0] ? new Grupo(result.rows[0]) : null;
  }

  async findByPeriodo(periodo) {
    const result = await query('SELECT * FROM grupos WHERE periodo = $1 AND activo = TRUE', [periodo]);
    return result.rows.map(r => new Grupo(r));
  }
  ...
  async create({ codigo, capacidad, jornada, asignaturaId, docenteId, periodo }) {
    ...
    return new Grupo(result.rows[0]);
  }
